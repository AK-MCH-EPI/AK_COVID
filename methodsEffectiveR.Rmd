...
Currently we right censor 6 days from the last reported date as this is the average incubation period, and then substitute the 
fitted values from a log-linear model based on 21 day period prior to the censor date. These "provisional" estimates enable us
to estimate Rt when cases have yet to be reported.
...
We initially provided both the parametric and empirical methods for estimating the serial interval distribution. Due to the limited
differences in estimates the empirical method was dropped due to the extensive computing time required. Finally, these estimates are made using a 14-day sliding window to reduce the impact of day-to-day variability.     

...
Briefly, using the estimate_R function in the EpiEstim package, a vector containing the incidence time series of cases based on onset date, estimates of the mean serial interval, and dates are supplied. Due to the delay between an individual recognizing symptoms, being tested, and those result being reported to the State, when an onset date is not yet know, the current practice is to substitute the reported date. To account for the fact that most of these substitution dates occur during the most recent week, a simple imputation method was developed to assign a temporary onset date when unknown that is sampled from a gamma distribution with a median of 6 days. This helps guard against making estimations that are attenuated due to limitations in data collection opposed to attenuate related to the disease transmission. Additionally, due to future cases that are yet to be reported likely having onset dates during the most recent week of collection, correction methods or truncation is also required.  

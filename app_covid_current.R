                      ...
                      tabPanel("Reproductive number",
                               sidebarPanel(
                                 ...
                                 helpText("NOTE:",
                                          ...
                                          br(),
                                          "The mean serial interval is the duration (in days) between the onset",
...
server <- shinyServer(function(input, output) {
  ...
  # Dates changed format on 4/28 update, and back again on 5/1  
  ...
  ## First create a new recovery date for the imputed version
  
  dat1$RecoverDateEst <- with(dat1,ifelse(!is.na(RecoverDate),dat1$RecoverDate,
                                          ifelse(!is.na(DeceasedDate), NA,
                                                 ifelse(!is.na(HospDate)+14 <= today(), dat1$HospDate+28,
                                                        ifelse(OnsetDate1+21 <= today(),dat1$OnsetDate1+21,
                                                               NA)))))
  dat1$RecoverDateEst <- as.Date((dat1$RecoverDateEst),origin = "1970-01-01")
  ...
  rpd <- as.data.frame(table(dat1$ReportDate))  # report date
  rpd$Var1 <- as.Date(rpd$Var1)
  rcv <- as.data.frame(table(dat1$RecoverDate))  # recover date
  rcv$Var1 <- as.Date(rcv$Var1)
  rcv1 <- as.data.frame(table(dat1$RecoverDateEst))  # recover date Estimated
  rcv1$Var1 <- as.Date(rcv1$Var1)  
  dth <- as.data.frame(table(dat1$DeceasedDate))  # death date
  dth$Var1 <- as.Date(dth$Var1)  
  ...
  dat2 <- merge(dat2, rpd, by.x = "date", by.y = "Var1", all.x = T)
  colnames(dat2)[4] <-"reportcnt"
  dat2 <- merge(dat2, osd1, by.x = "date", by.y = "Var1", all.x = T)
  colnames(dat2)[5] <-"onsetcnt1" 
  dat2 <- merge(dat2, rcv, by.x = "date", by.y = "Var1", all.x = T)
  colnames(dat2)[6] <-"rcvrcnt"  
  dat2 <- merge(dat2, rcv1, by.x = "date", by.y = "Var1", all.x = T)
  colnames(dat2)[7] <-"rcvrcntest" 
  dat2 <- merge(dat2, dth, by.x = "date", by.y = "Var1", all.x = T)
  colnames(dat2)[8] <-"dthcnt" 
  ...
  dat2$reportsum <- cumsum(dat2$reportcnt)
  dat2$recoversum <- cumsum(dat2$rcvrcnt)
  dat2$recoverEstsum <- cumsum(dat2$rcvrcntest)
  dat2$deathsum <- cumsum(dat2$dthcnt)
  
  # Current active cases by day
  ## A = I - D - R
  
  dat2$activesum <- dat2$reportsum - dat2$deathsum - dat2$recoversum
  # Active cases imputed recovered date
  dat2$activesumEst <- dat2$reportsum - dat2$deathsum - dat2$recoverEstsum
  ...
  output$Plot_r <- renderPlotly({
    ...
    dfx <- data.frame(mean = mean, low = low, high = high, tnk = as.Date(tnk, origin = "1970-01-01"))
    
    a <- ggplot(dfx, aes(x=tnk, y=mean))+
      ...
      ylim(0,4)+
      theme_minimal()+
      geom_ribbon(data = dfx, aes(x = tnk, ymin = low, ymax = high), 
                  alpha = 0.15,fill = "#0a306a")+
      labs(title="Time-varying reproductive number (Rt), Alaska")  +
      scale_x_date(breaks = pretty( dfx$tnk, n = 10),date_labels = "%d %b") 
